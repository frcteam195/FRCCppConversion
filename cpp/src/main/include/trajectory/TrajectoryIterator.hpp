...
namespace ck
{
    namespace trajectory
    {
        ...
        public:
            TrajectoryIterator(const TrajectoryView<S> &view) : view_(&view), progress_(view.first_interpolant()), current_sample_(view.sample(view.first_interpolant())) {}
            bool isDone() { return getRemainingProgress() == 0.0; }
            double getProgress() { return progress_; }
            double getRemainingProgress() { return ck::math::max(0.0, view_->last_interpolant() - progress_); }
            TrajectorySamplePoint<S> getSample() { return current_sample_; }
            S getState() { return current_sample_.state(); }
