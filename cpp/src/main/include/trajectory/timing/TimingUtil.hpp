        ...
        namespace timing
        {
            ...
            class TimingUtil
            {
                ...
                static Trajectory<TimedState<S>> timeParameterizeTrajectory(
                    bool reverse,
                    const DistanceView<S> &distance_view,
                    double step_size,
                    const std::vector<TimingConstraint<S>> &constraints,
                    ...
                    double max_abs_acceleration)
                {
                    ...
                    {
                        states.push_back(distance_view.sample(ck::math::min(i * step_size, distance_view.last_interpolant())).state());
                    }
                ...
                static Trajectory<TimedState<S>> timeParameterizeTrajectory(
                    ...
                    double max_abs_acceleration)
                {
                    std::vector<ConstrainedState<S>> constraint_states(states.size());
                    ...
                    for (size_t i = 0; i < states.size(); ++i)
                    {
                        ...
                        while (true)
                        {
                            ...
                            constraint_state.max_velocity = Math.min(max_velocity,
                                                                     Math.sqrt(predecessor.max_velocity * predecessor.max_velocity + 2.0 * predecessor.max_acceleration * ds));
