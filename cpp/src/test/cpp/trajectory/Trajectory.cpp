...
TEST(TrajTest, Construction){
    ...
    traj =
        ck::trajectory::Trajectory<ck::geometry::Translation2d>(kWaypoints);
...
TEST(TrajTest, StateAccessor){
    ...
    ASSERT_EQ( kWaypoints[0].interpolate(kWaypoints[1], .25), index_view.sample(0.25).state() );
    ASSERT_EQ( kWaypoints[1].interpolate(kWaypoints[2], .5), index_view.sample(1.5).state() );
